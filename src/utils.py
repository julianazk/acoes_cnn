...
def download_save(url, path_to_save):
    print("Comecando download " + re.sub(r'apikey=[A-Za-z0-9]+&', 'apikey=my_api_key&', url))
    urllib.request.urlretrieve(url, path_to_save)
    print(path_to_save + " download concluido e salvo")
...
def save_array_as_images(x, img_width, img_height, path, file_names, y):
    if os.path.exists(path):
        shutil.rmtree(path)
        print("arquivos antigos apagados")

    os.makedirs(path)
    print("Pasta de imagens criada", path)
    ...
    for i in tqdm(range(x.shape[0])):
        ...
        img = Image.fromarray(x_temp[i], 'L')
        if (y[i] == 1) :
            strLabel = '_buy'
        elif (y[i] == 0):
            strLabel = '_sell'
        ...
        img.save(os.path.join(path, str(file_names) + str(i) + strLabel + '.png'))

    print_time("Imagens salvas em " + path, stime)
    return x_temp
